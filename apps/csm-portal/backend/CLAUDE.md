...
- **`internal/caseevents.Handler`** — the consumer side, **wired into `cmd/server/main.go`** (started whenever `EVENT_HUB_BROKER` is set, in its own consumer group — `EVENT_HUB_CONSUMER_GROUP`, suffixed `-replica-<uuid>` per process so every replica sees every event rather than Kafka load-balancing them across replicas). Logs `type`/`entityId` for every event (never the payload — it can carry PII, e.g. comment text or recipient emails), and for `case.comment_added`/`case.status_changed` specifically also fans a minimal `{caseId, type, timestamp}` payload out to `internal/stream.BroadcastHub` — see "Case-activity SSE stream" below, which is what that scaffold grew into.
...
- **This listener alone runs `middleware.CORS`** (`STREAM_CORS_ALLOWED_ORIGINS` env var, comma-separated; unset allows any origin), outermost in its chain. The `:8080` API doesn't need this in-app — Choreo's gateway adds CORS for it — but a browser XHR/fetch call (unlike a WebSocket handshake) is subject to CORS, and a freshly-added second Choreo endpoint isn't guaranteed to inherit gateway CORS the same way, so this backend handles it directly rather than assuming.
