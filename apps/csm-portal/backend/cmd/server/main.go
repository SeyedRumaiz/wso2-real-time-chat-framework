...
func main() {
	...
	customerPortalHandler := handler.NewCustomerPortalHandler(customerEntityClient)
	...
	mux.HandleFunc("GET /metadata", customerPortalHandler.GetMetadata)
	mux.HandleFunc("POST /search", customerPortalHandler.GlobalSearch)
	...
	mux.HandleFunc("GET /cases/{id}/feedback", customerPortalHandler.GetCaseFeedback)
	mux.HandleFunc("POST /cases/{id}/feedback", customerPortalHandler.SubmitCaseFeedback)
	mux.HandleFunc("GET /cases/{id}/attachments", customerPortalHandler.GetCaseAttachments)
	mux.HandleFunc("POST /cases/{id}/attachments", customerPortalHandler.CreateCaseAttachment)
	mux.HandleFunc("PATCH /cases/{caseId}/attachments/{attachmentId}", customerPortalHandler.PatchCaseAttachment)
	mux.HandleFunc("POST /cases/{caseId}/escalations", customerPortalHandler.CreateCaseEscalation)
	mux.HandleFunc("POST /cases/{caseId}/escalations/search", customerPortalHandler.SearchCaseEscalations)
	...
	mux.HandleFunc("GET /attachments/{id}", customerPortalHandler.GetAttachment)
	...
	mux.HandleFunc("POST /projects/{id}/instances/search", customerPortalHandler.SearchProjectInstances)
	mux.HandleFunc("POST /projects/{id}/conversations/search", customerPortalHandler.SearchProjectConversations)
	mux.HandleFunc("POST /projects/{id}/conversations", customerPortalHandler.CreateProjectConversation)
	mux.HandleFunc("POST /projects/{id}/cases/time-cards/search", customerPortalHandler.SearchProjectCaseTimeCards)
	mux.HandleFunc("POST /projects/{id}/instances/metrics/search", customerPortalHandler.SearchProjectInstanceMetrics)
	mux.HandleFunc("POST /projects/{id}/instances/usages/search", customerPortalHandler.SearchProjectInstanceUsage)
	mux.HandleFunc("POST /projects/{id}/instances/stats/metrics/search", customerPortalHandler.SearchProjectInstanceMetricsStats)
	mux.HandleFunc("POST /projects/{id}/instances/stats/usages/search", customerPortalHandler.SearchProjectInstanceUsageStats)
	...
	mux.HandleFunc("GET /deployments/{id}/attachments", customerPortalHandler.GetDeploymentAttachments)
	mux.HandleFunc("POST /deployments/{id}/attachments", customerPortalHandler.CreateDeploymentAttachment)
	mux.HandleFunc("PATCH /deployments/{deploymentId}/attachments/{attachmentId}", customerPortalHandler.PatchDeploymentAttachment)
	mux.HandleFunc("POST /deployments/{id}/instances/search", customerPortalHandler.SearchDeploymentInstances)
	mux.HandleFunc("POST /deployments/{id}/instances/metrics/search", customerPortalHandler.SearchDeploymentInstanceMetrics)
	mux.HandleFunc("POST /deployments/{id}/instances/usages/search", customerPortalHandler.SearchDeploymentInstanceUsage)
	mux.HandleFunc("POST /deployments/{id}/instances/stats/metrics/search", customerPortalHandler.SearchDeploymentInstanceMetricsStats)
	mux.HandleFunc("POST /deployments/{id}/instances/stats/usages/search", customerPortalHandler.SearchDeploymentInstanceUsageStats)
	...
	mux.HandleFunc("POST /deployments/products/{id}/instances/search", customerPortalHandler.SearchDeployedProductInstances)
	mux.HandleFunc("POST /deployments/products/{id}/instances/metrics/search", customerPortalHandler.SearchDeployedProductInstanceMetrics)
	mux.HandleFunc("POST /deployments/products/{id}/instances/usages/search", customerPortalHandler.SearchDeployedProductInstanceUsage)
	mux.HandleFunc("POST /deployments/products/{id}/instances/stats/metrics/search", customerPortalHandler.SearchDeployedProductInstanceMetricsStats)
	mux.HandleFunc("POST /deployments/products/{id}/instances/stats/usages/search", customerPortalHandler.SearchDeployedProductInstanceUsageStats)
	mux.HandleFunc("POST /deployments/{deploymentId}/products/{productId}/metrics/search", customerPortalHandler.SearchDeployedProductMetrics)
	mux.HandleFunc("POST /deployments/{deploymentId}/products/{productId}/metrics/usage-counts/search", customerPortalHandler.SearchDeployedProductUsageCounts)
	...
	mux.HandleFunc("GET /products/vulnerabilities/meta", customerPortalHandler.GetProductVulnerabilityMetadata)
	...
	mux.HandleFunc("GET /conversations/{id}", customerPortalHandler.GetConversation)
	mux.HandleFunc("PATCH /conversations/{id}", customerPortalHandler.UpdateConversation)
